...
function initEncryptPanel() {
    ...
    $('enc-clear').addEventListener('click', () => {
        ...
        output.innerHTML = '<span class="output-placeholder">Encrypted text will appear here…</span>';
    });
    ...
    $('enc-btn').addEventListener('click', async () => {
        ...
        if (cipher === 'affine') { body.a = parseInt($('enc-a').value) || 5; body.b = parseInt($('enc-b').value) || 8; }
        if (cipher === 'substitution') { body.key = $('enc-key').value.trim(); }
...
function initDecryptPanel() {
    ...
    $('dec-load-file').addEventListener('click', () => loadFile($('dec-file-input'), input));
    $('dec-clear').addEventListener('click', () => {
        input.value = '';
        $('dec-char-count').textContent = '0';
        output.innerHTML = '<span class="output-placeholder">Decrypted text will appear here…</span>';
        $('dec-detected-badge').classList.add('hidden');
    ...
    $('dec-btn').addEventListener('click', async () => {
        const text = input.value.trim();
        if (!text) { showToast('Please paste some ciphertext first', 'error'); return; }
        ...
        if (cipher === 'caesar' && method === 'key') body.shift = parseInt($('dec-shift').value) || 3;
        if (cipher === 'affine' && method === 'key') { body.a = parseInt($('dec-a').value) || 5; body.b = parseInt($('dec-b').value) || 8; }
        if (cipher === 'substitution' && method === 'key') body.key = $('dec-key').value.trim();
        ...
        try {
            ...
            // Show detected badge if auto
            const badge = $('dec-detected-badge');
            let finalLang = language;
            if (data.detected_cipher || data.detected_language) {
                finalLang = data.detected_language || language;
                const ct = data.detected_cipher || cipher;
                badge.textContent = `${ct} · ${finalLang}`;
            ...
            try {
                const freqData = await apiCall('/api/frequency', { text: data.result, language: finalLang });
                ...
                renderFrequencyChart('dec-freq-chart', labels, observed, reference, freqData.language);
            } catch (err) {
...
function updateDecParams() {
    const cipher = $('dec-cipher').value;
    const method = $('dec-method').value;
    const isKey = method === 'key' && cipher !== 'auto';

    $('dec-caesar-params').classList.toggle('hidden', !(isKey && cipher === 'caesar'));
    $('dec-affine-params').classList.toggle('hidden', !(isKey && cipher === 'affine'));
    $('dec-sub-params').classList.toggle('hidden', !(isKey && cipher === 'substitution'));

    // Auto cipher forces frequency method
    if (cipher === 'auto') {
        $('dec-method').value = 'frequency';
        $('dec-method').disabled = true;
    } else {
        $('dec-method').disabled = false;
    }
}
...
function initFrequencyPanel() {
    ...
    $('freq-auto-detect-btn').addEventListener('click', async () => {
        const text = $('freq-input').value.trim();
        if (!text) { showToast('Enter text to auto-detect language', 'error'); return; }
        try {
            const data = await apiCall('/api/detect-language', { text });
            $('freq-language').value = data.language;
            showToast(`Language detected: ${data.language} ✓`, 'info');
        } catch (e) {
            showToast(e.message, 'error');
        }
    });
    ...
    $('freq-btn').addEventListener('click', async () => {
        ...
        if (!text) { showToast('Enter some text to analyse', 'error'); return; }
        ...
        try {
            ...
            renderFrequencyChart('freq-chart', labels, observed, reference, data.language);

            // Stats
            const uniqueLetters = Object.values(data.observed).filter(v => v > 0).length;
            const topLetter = labels.reduce((a, b) => (data.observed[a] || 0) > (data.observed[b] || 0) ? a : b);
            $('freq-stats').innerHTML = `
        <div class="freq-stat">
...
function initSettingsPanel() {
    ...
    $('settings-default-lang').addEventListener('change', e => state.settings.defaultLang = e.target.value);
    $('settings-default-cipher').addEventListener('change', e => state.settings.defaultCipher = e.target.value);
}
