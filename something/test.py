...
class Key(object):
    def __init__(self, key_str, alphabet=None):
        if alphabet is None:
            alphabet = "ابتثجحخدذرزسشصضطظعغفقكلمنهوي"
        self.alphabet = alphabet.lower()
        self.key = key_str.lower()
        if len(self.key) != len(self.alphabet):
            raise ValueError("Key length must match alphabet")
        if sorted(self.key) != sorted(self.alphabet):
            raise ValueError("Key must be permutation")

    def decode(self, text):
        result = ""
        for char in text:
            lower_char = char.lower()
            if lower_char in self.key:
                new_char = self.alphabet[self.key.index(lower_char)]
                if char.isupper():
...
class Breaker(object):
    ...
    :type key: :class:`Key`
    :param quadgram_file_path: path to the quadgram JSON file
    :type quadgram_file_path: str
    """

    _DEFAULT_ALPHABET = "abcdefghijklmnopqrstuvwxyz"

    def __init__(self, quadgram_file_path):
        """Init the instance
        """
        with open(quadgram_file_path, 'r', encoding='utf-8') as quadgram_fh:
            obj = json.load(quadgram_fh)
        
        # Detect n from the length of the first key
        if obj:
            first_key = next(iter(obj.keys()))
            self._n = len(first_key)
            # Extract unique characters from all ngrams to determine the alphabet
            all_chars = set()
            for gram in obj.keys():
                for char in gram:
                    all_chars.add(char.lower())
            self._alphabet = ''.join(sorted(all_chars))
        else:
            self._n = 4  # default
            self._alphabet = Breaker._DEFAULT_ALPHABET
        
        self._alphabet_len = len(self._alphabet)
        
        # Initialize ngrams list
        ngrams = [0] * (32 ** self._n)
        ...
        for gram, count in obj.items():
            if len(gram) != self._n:
                continue
            idx = 0
            for char in gram.lower():
                if char in self._alphabet:
                    idx = (idx << 5) + self._alphabet.index(char)
                else:
                    break
            else:
                ngrams[idx] = count
        
        # Normalize as in generate_quadgrams
        gram_sum = sum(ngrams)
    ...
    def _calc_fitness(self, iterator):
        ...
        :return: the fitness of the text. A value close to 100 means, the
            ...
            the ngrams. The more the value differs from 100, the lesser the
            probability that the examined text corresponds to the ngram language.
        ...
        try:
            ...
            raise ValueError(
                f"More than {self._n - 1} characters from the given alphabet are required"
            )
        ...
        nbr_quadgrams += 1
        for numerical_char in iterator:
            quadgram_val = ((quadgram_val & self._mask) << 5) + numerical_char
            fitness += quadgrams[quadgram_val]
            nbr_quadgrams += 1
        if nbr_quadgrams == 0:
            raise ValueError(
        ...
        while better_key:
            ...
            for idx1 in range(key_len - 1):
                for idx2 in range(idx1 + 1, key_len):
                    ...
                    for char in plaintext[self._n - 1:]:
                        quad_idx = ((quad_idx & self._mask) << 5) + char
    ...
    def break_cipher(self, ciphertext, max_rounds=10000, consolidate=3):
        ...
        if len(cipher_bin) < self._n:
            raise ValueError(f"ciphertext is too short, needs at least {self._n} characters")
        ...
        return BreakerResult(
            ...
            key=key_str,
            alphabet=self._alphabet,
            fitness=local_maximum / (len(cipher_bin) - self._n + 1) / 10,
            nbr_keys=nbr_keys,
